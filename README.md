...
All numbers below are **genuine** — computed by the evaluation pipeline, never
hardcoded. The full A1–A15 ablation lives in
[`outputs/results/ablation.json`](outputs/results/ablation.json); the thesis
tables and figures are auto-generated by `scripts/build_thesis_tables.py` and
`scripts/build_thesis_figures.py`.

| Configuration | Tier 2 backbone | AUC | Accuracy | ECE |
|---|---|:--:|:--:|:--:|
| A1 — Tier 1 only (MobileNetV2) | — | 0.918 | 0.855 | 0.086 |
| A6 — Tiered + MC + TTA | EfficientNet-B4 | 0.911 | 0.888 | 0.050 |
| A13 — Proposed tiered | Ark+ Swin | 0.923 | 0.828 | — |
| **A15 — Proposed + Mixup/CutMix** | **Ark+ Swin** | **0.924** | **0.882** | **0.060** |

- The lightweight **Tier 1 resolves ≈ 74 %** of cases; only ≈ 26 % escalate to the heavy Tier 2.
- Adding MC-Dropout + TTA cuts calibration error **ECE 0.074 → 0.050** with no loss in AUC.
- Conformal prediction reaches **97.3 % empirical coverage** at a 95 % target.
- Zero-shot on CheXpert (preliminary), AUC holds at **0.885** vs 0.923 in-domain.

![Ablation overview (AUC and ECE for A1–A15)](thesis/figures/ablation_overview.png)
