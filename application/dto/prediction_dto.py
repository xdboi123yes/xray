...
class PredictionDTO(BaseModel):
    ...
    prediction: str = Field(
        ..., description="Final diagnostic decision: 'Pneumothorax' or 'No Finding'."
    )
    confidence: float = Field(
        ..., description="Prediction probability confidence score between 0.0 and 1.0."
    )
    tier_used: int = Field(..., description="Classifier tier utilized: 1 (MobileNet) or 2 (Ark+).")
    mc_variance: float | None = Field(
        None, description="Monte Carlo Dropout variance for uncertainty rating."
    )
    mc_passes: int | None = Field(
        None, description="Number of Monte Carlo dropout forward passes run."
    )
    tta_passes: int | None = Field(None, description="Number of Test-Time Augmentation passes run.")
    conformal_set: list[str] | None = Field(
        None, description="Set prediction boundaries calculated via conformal calibration."
    )
    conformal_coverage: float | None = Field(
        None, description="Nominal coverage guarantees (e.g. 0.95)."
    )
    flagged_for_review: bool = Field(
        ..., description="Whether the sample was flagged due to extreme high uncertainty."
    )
    inference_time_ms: float = Field(
        ...,
        description="Total wall-clock duration in milliseconds for preprocessing and inference.",
    )
    gradcam_tier1_b64: str | None = Field(
        None, description="Base64-encoded Grad-CAM diagnostic heatmap from Tier 1."
    )
    gradcam_tier2_b64: str | None = Field(
        None, description="Base64-encoded Grad-CAM diagnostic heatmap from Tier 2."
    )
    model_version: str = Field(
        ..., description="Active versions of the deployed diagnostic models."
    )
    timestamp: str = Field(
        ..., description="ISO 8601 UTC timestamp of the completed diagnostic request."
    )
...
class HealthDTO(BaseModel):
    ...
    gpu: bool = Field(
        ..., description="Whether a GPU/MPS accelerator is active and registered by PyTorch."
    )
    models_loaded: list[str] = Field(
        ..., description="Identifiers of currently instantiated model layers in memory."
    )
...
class HistoryRecordDTO(BaseModel):
    ...
    flagged_for_review: bool = Field(
        ..., description="Whether flagged for visual clinician oversight."
    )
