...
class TrainingConfigDTO(BaseModel):
    ...
    lr_backbone: float = Field(
        1e-4, gt=0.0, description="Optimizer learning rate for base features."
    )
    lr_head: float = Field(
        1e-3, gt=0.0, description="Optimizer learning rate for classifier heads."
    )
    early_stopping_patience: int = Field(
        7, ge=1, description="Patience epochs before early termination."
    )
    ...
    num_workers: int = Field(
        0,
        ge=0,
        description="Number of workers for multiprocess dataloading (0 for single-process).",
    )
